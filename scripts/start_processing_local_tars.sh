...
# --- Local archive processing (manual copy) ---
if [ -d "$local_archive_dir" ]; then
    echo "Processing local archives from $local_archive_dir"
    all_tar_files=( $(ls "$local_archive_dir"/*.tar.bz2 2>/dev/null) )
    archive_bases=()
    for file_path in "${all_tar_files[@]}"; do
    ...
    for tar_file_name in "${processed_archives[@]}"; do
      echo "Processing local $tar_file_name"
      tar_file="$local_archive_dir/$tar_file_name"
      cp "$tar_file" "$archive_files" # Copy to working archive_files dir
      ...
      echo "Tar processing completed (local)"
      ...
      # Remove original local archive after successful processing
      rm -f "$local_archive_dir/$tar_file_name"
    done
fi
